{{--
    ...
    Sections: toast, summary cards, filter toolbar, desktop table +
    mobile card fallback, pagination, Add/Edit modal, confirm modal.
--}}
@php
...
<x-dashboard-layout title="User Management">
    ...
    {{-- Toast: session flash, auto-dismissed by admin-user-management.js --}}
    @if (session('success'))
    ...
    <div class="grid grid-cols-2 gap-4 mb-6 sm:grid-cols-3 lg:grid-cols-5">
        <div class="rounded-xl border border-assocmap-border bg-white p-4 shadow-card">
            ...
            <p class="mt-1 text-2xl font-bold text-assocmap-text">{{ $summary['total'] }}</p>
        </div>
        <div class="rounded-xl border border-assocmap-border bg-white p-4 shadow-card">
            <p class="text-xs font-medium text-assocmap-secondary">System Admins</p>
            <p class="mt-1 text-2xl font-bold text-green-700">{{ $summary['admins'] }}</p>
        </div>
        <div class="rounded-xl border border-assocmap-border bg-white p-4 shadow-card">
            <p class="text-xs font-medium text-assocmap-secondary">Field Officers</p>
            <p class="mt-1 text-2xl font-bold text-blue-700">{{ $summary['field_officers'] }}</p>
        </div>
        <div class="rounded-xl border border-assocmap-border bg-white p-4 shadow-card">
            <p class="text-xs font-medium text-assocmap-secondary">Assoc. Members</p>
            <p class="mt-1 text-2xl font-bold text-purple-700">{{ $summary['members'] }}</p>
        </div>
        <div class="rounded-xl border border-assocmap-border bg-white p-4 shadow-card">
            <p class="text-xs font-medium text-assocmap-secondary">Inactive</p>
            <p class="mt-1 text-2xl font-bold text-red-600">{{ $summary['inactive'] }}</p>
        </div>
    </div>

    {{-- Filter toolbar --}}
    <form method="GET" action="{{ route('users.index') }}"
          class="mb-6 flex flex-wrap items-end gap-3 rounded-xl border border-assocmap-border bg-white p-4 shadow-card">
        ...
        <div>
            ...
            <select name="role_id" class="mt-1 rounded-lg border border-assocmap-border px-3 py-2 text-sm">
        ...
        <div>
            ...
            <select name="status" class="mt-1 rounded-lg border border-assocmap-border px-3 py-2 text-sm">
        ...
        <div>
            ...
            <select name="sort" class="mt-1 rounded-lg border border-assocmap-border px-3 py-2 text-sm">
        ...
        <button type="button" data-admin-user-modal-open="create"
                class="ml-auto rounded-lg bg-assocmap-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-assocmap-hover">
            + Add User
        </button>
    </form>
    ...
    <div class="hidden overflow-x-auto rounded-xl border border-assocmap-border bg-white shadow-card md:block">
        <table class="min-w-full divide-y divide-assocmap-border text-sm">
            <thead class="bg-assocmap-bg">
                <tr>
                    <th class="px-4 py-3 text-left font-semibold text-assocmap-text">User</th>
            ...
            <tbody class="divide-y divide-assocmap-border">
                @forelse ($users as $user)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-assocmap-secondary">{{ $user->created_at->format('M d, Y') }}</td>
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-2 whitespace-nowrap">
                                <button type="button"
                                        data-admin-user-modal-open="edit"
                                        data-user='@json($user)'
                                        class="rounded-md border border-assocmap-border px-3 py-1.5 text-xs font-semibold text-assocmap-text hover:bg-assocmap-bg">
                                    Edit
                                </button>
                                ...
                                <button type="button"
                                        ...
                                        data-confirm-message="{{ $user->is_active ? 'This user will no longer be able to log in. This does not delete their record.' : 'This user will be able to log in again.' }}"
                                        data-confirm-label="{{ $user->is_active ? 'Deactivate' : 'Reactivate' }}"
                                        class="rounded-md border border-assocmap-border px-3 py-1.5 text-xs font-semibold {{ $user->is_active ? 'text-red-600 hover:bg-red-50' : 'text-green-600 hover:bg-green-50' }}">
            ...
            <div class="rounded-xl border border-assocmap-border bg-white p-4 shadow-card">
                ...
                <div class="mt-3 flex gap-2">
                    ...
                    <button type="button"
                            ...
                            data-confirm-title="{{ $user->is_active ? 'Deactivate User?' : 'Reactivate User?' }}"
                            data-confirm-message="{{ $user->is_active ? 'This user will no longer be able to log in.' : 'This user will be able to log in again.' }}"
                            data-confirm-label="{{ $user->is_active ? 'Deactivate' : 'Reactivate' }}"
            ...
            <form id="admin-user-form" method="POST" action="{{ route('users.store') }}">
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        {{-- Informational only - the real guard runs server-side --}}
    ...
    {{-- Generic confirm dialog - reused for deactivate/reactivate --}}
